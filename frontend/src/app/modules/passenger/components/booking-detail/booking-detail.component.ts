...
export class BookingDetailComponent implements OnInit {
    ...
    this.bookingService.getBookingById(this.bookingId).subscribe({
      next: (response) => {
        ...
        if (response.success && response.data) {
          ...
          this.error = response.message || 'Errore nel caricamento della prenotazione';
        }
      },
      error: (error) => {
        this.loading = false;
        console.error('Error loading booking detail:', error);
        this.error = error.error?.message || 'Errore nel caricamento della prenotazione';
      }
    });
  }
    ...
    this.bookingService.checkIn(this.booking._id).subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.snackBar.open(response.message || 'Errore durante il check-in', 'Chiudi', {
            duration: 3000,
      ...
      error: (error) => {
        ...
        this.snackBar.open(error.error?.message || 'Errore durante il check-in', 'Chiudi', {
          duration: 3000,
          panelClass: ['error-snackbar']
        });
      }
        ...
        this.bookingService.cancelBooking(this.booking._id).subscribe({
          next: (response) => {
            if (response.success) {
              ...
              this.snackBar.open('Prenotazione cancellata con successo', 'Chiudi', {
                duration: 3000,
                panelClass: ['success-snackbar']
              });
            } else {
              this.snackBar.open(response.message || 'Errore durante la cancellazione', 'Chiudi', {
                duration: 3000,
                panelClass: ['error-snackbar']
              });
            }
          ...
          error: (error) => {
            ...
            this.snackBar.open(error.error?.message || 'Errore durante la cancellazione', 'Chiudi', {
              duration: 3000,
              panelClass: ['error-snackbar']
            });
          }
  ...
  canCheckIn(): boolean {
    if (!this.booking || this.booking.checkedIn || this.booking.status !== 'confirmed' || this.booking.paymentStatus !== 'paid') {
      return false;
    }
  ...
  canCancel(): boolean {
    if (!this.booking || this.booking.status === 'cancelled' || this.booking.status === 'completed') {
      return false;
    }
