...
const aircraftSchema = new Schema<IAircraft>({
  registrationNumber: {
    ...
    trim: true,
    uppercase: true,
    match: [/^[A-Z]{1,2}-[A-Z0-9]{3,6}$/, 'Invalid registration number format']
  },
  airline: {
...
aircraftSchema.pre('save', function(next) {
  ...
  if (this.maintenance.nextMaintenanceDate <= this.maintenance.lastMaintenanceDate) {
    const error = new Error('Next maintenance date must be after last maintenance date');
    return next(error);
  }
  
  // Calculate total capacity from seat configuration
  if (this.seatConfiguration && this.seatConfiguration.length > 0) {
    const calculatedCapacity = this.seatConfiguration.reduce((total, config) => {
