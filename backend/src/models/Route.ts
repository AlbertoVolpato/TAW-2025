...
const routeSchema = new Schema<IRoute>({
  routeCode: {
    ...
    trim: true,
    uppercase: true,
    match: [/^[A-Z]{2}\d{3,4}$/, 'Route code must follow format: XX123 or XX1234']
  },
  airline: {
...
routeSchema.pre('save', function(next) {
  if (this.origin.equals(this.destination)) {
    const error = new Error('Origin and destination airports must be different');
    return next(error);
  }
  
  // Validation: end date must be after start date
  if (this.seasonality.endDate <= this.seasonality.startDate) {
    const error = new Error('Season end date must be after start date');
    return next(error);
  }
